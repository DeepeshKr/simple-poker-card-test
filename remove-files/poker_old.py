...
class Card (object):
  ...
  # SUITS = ('S', 'D', 'H', 'C') # SPADE DAIMOND HEART CLUB
...
class Poker (object):
  ...
  def __init__ (self, numHands):
    ...
    for i in range (numHands):
      ...
      print('Line 79 self.deck : ' + ' '.join(map(str, hand))) # remove before sending

  def play (self):
    for i in range (len (self.hands) ):
      print('Line 85 self.deck : ' + str(len (self.hands))) # remove before sending
      sortedHand = sorted (self.hands[i], reverse = True)
      print('Line 87 self.deck : ' + ' '.join(map(str, sortedHand))) # remove before sending
      hand = ''
  ...
  def point(self,hand):                         #point()function to calculate partial score
    ...
    print('Line 97 c_sum : ' + str(c_sum)) # remove before sending
    return c_sum
  ...
  def isRoyal (self, hand):               #returns the total_point and prints out 'Royal Flush' if true, if false, pass down to isStraightFlush(hand)
    sortedHand=sorted(hand,reverse=True)
    print('Sorted Hand : ' + ' '.join(map(str, sortedHand))) # remove before sending
    flag=True
    h=10
...
def main ():
  numHands = 2 #eval (input ('Enter number of hands to play: '))
  # while (numHands < 2 or numHands > 6):
  #   numHands = eval( input ('Enter number of hands to play: ') )
  print('Starting 2 player Poker Game')
  game = Poker (numHands)
  game.play()  
